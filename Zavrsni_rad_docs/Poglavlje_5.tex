...
Sada se možemo vratiti na situaciju iz \ref{fig:26}. k-d stablo će nam onakav prostor podijeliti na sljedeći način\cite{14}:
\begin{figure}[!http]
	\begin{center}
		\begin{tikzpicture}
...
\end{algorithm}
\newpage
Sada kada znamo izgraditi stablo možemo prikazati kako bi stablo konkretno izgledalo za situaciju \ref{fig:27} it prošlog poglavlja:
\begin{figure}[!http]
	\begin{center}
		\begin{tikzpicture}[level distance=1.5cm,
		level 1/.style={sibling distance=3cm},
		...
		\end{tikzpicture}
	\end{center}
	\caption {Prikaz strukture k-d stabla za \ref{fig:27}}
	\label{fig:28}
\end{figure}
